(function($) {
	...
	$.fn.flapper = function(options) {
        this.each(function(){
            ...
            var t = new Flapper($(this), options);
            sample.push(t);
...
// This is custom code to allow us change the flap speed during the animation 
    $.fn.flapper2 =  function (newTiming, newMinTiiming, newThreshold){
        sample.forEach(function(t){
            var timing =  parseInt(newTiming);
            var minTiming =  parseInt(newMinTiiming);
            var threshold =  parseInt(newThreshold);
            t.options.min_timing = minTiming;
            t.options.timing = timing;
            t.options.threshhold = threshold;
        });
    };
